...
def run_training_cycle(
    ...
    Worker assignment:
      * If thread_groups_override is supplied, use those configured named
        worker groups exactly. This is the normal path from main(), where
        [threads] mode definitions are resolved once and shared by cycle stages.
      * If no override is supplied and thread_mode is numeric, split the
        configured agent list into that many contiguous fallback chunks.
      * thread_mode == "1" runs inline in the main process; other numeric
        modes use one OS process per nonempty group.
...
def _cycle_rr_thread_group_chunks(
    agent_names: Sequence[str],
    thread_groups_override: Optional[Sequence[Sequence[str]]],
    thread_mode: str,
) -> List[List[int]]:
    """Return cycle-RR White-row ownership as lists of agent indices.

    The post-rotation champion RR is logically another cycle stage, so named
    thread modes should own the same agents here as they do during GA training.
    Only the prelude keeps its special seed-row worker split.

    If no explicit named mode is supplied, numeric thread modes retain the old
    contiguous row-split fallback.
    """
    ...
    if thread_groups_override is not None:
        ...
        seen = set()
        for group in thread_groups_override:
            indices: List[int] = []
            for raw_name in group:
                name = str(raw_name)
                if name not in index_by_name:
                    raise ValueError(f"Cycle RR thread group contains unknown agent name: {name!r}")
                idx = index_by_name[name]
                if idx in seen:
                    raise ValueError(f"Cycle RR thread group contains duplicate agent name: {name!r}")
                seen.add(idx)
                indices.append(idx)
            if indices:
                chunks.append(indices)

        missing = [name for idx, name in enumerate(names) if idx not in seen]
        if missing:
            raise ValueError(f"Cycle RR thread groups do not cover all agents: {missing}")
        return chunks or [list(range(len(names)))]

    worker_count = _cycle_rr_worker_count_from_thread_mode(thread_mode, len(names))
    return [list(range(a, b)) for a, b in _make_cycle_rr_chunks(worker_count, len(names))]


def _cycle_rr_rows_label(rows: Sequence[int], agent_names: Optional[Sequence[str]] = None) -> str:
    row_nums = [int(x) + 1 for x in rows]
    if not row_nums:
        return "none"
    if row_nums == list(range(min(row_nums), max(row_nums) + 1)):
        base = f"{min(row_nums)}-{max(row_nums)}" if len(row_nums) > 1 else str(row_nums[0])
    else:
        base = ",".join(str(x) for x in row_nums)
    if agent_names is None:
        return base
    names = [str(agent_names[int(i)]) for i in rows]
...
def _empty_cycle_rr_worker_progress(
    *,
    cycle: int,
    worker_id: int,
    white_indices: Sequence[int],
    reps: int,
    agent_names: Sequence[str],
) -> Dict[str, object]:
    n = len(agent_names)
    rows = [int(x) for x in white_indices]
    total_games = int(reps) * len(rows) * n
    ...
    return {
        ...
        "white_indices": rows,
        "white_names": [str(agent_names[i]) for i in rows],
        # Backward-readable summary fields; schedule validation uses white_indices.
        "row_start": min(rows) if rows else 0,
        "row_end": (max(rows) + 1) if rows else 0,
        "reps": int(reps),
        "agent_names_sha1": _cycle_rr_names_sha1(agent_names),
        "game_index": 0,
...
def _cycle_rr_local_index_to_game(
    ...
    *,
    ...
    rows = [int(x) for x in white_indices]
    if not rows:
        raise ValueError("Cycle RR worker has no White rows assigned.")
    games_per_rep = len(rows) * int(n_agents)
    rep = int(local_index) // games_per_rep
    rem = int(local_index) % games_per_rep
    white_idx = rows[rem // int(n_agents)]
...
def _cycle_rr_reconcile_progress_with_worker_log(
    *,
    ...
    n = len(agent_names)
    rows = [int(x) for x in white_indices]
    total_games = int(progress.get("total_games", int(reps) * len(rows) * n) or 0)
    ...
    progress["white_indices"] = rows
    progress["white_names"] = [str(agent_names[i]) for i in rows]
    progress["row_start"] = min(rows) if rows else 0
    progress["row_end"] = (max(rows) + 1) if rows else 0
    progress["total_games"] = int(total_games)
    progress["games_total"] = int(total_games)
...
def _cycle_rr_worker_row_block(
    *,
    ...
    Each worker owns its configured set of White champion rows.  It plays
    ...
    try:
        ...
        if bool(progress.get("done")) and local_games >= total_games:
            log(f"[cycle {cycle} RR] worker {worker_id} white rows {rows_label} already done ({local_games}/{total_games}).")
        else:
            log(f"[cycle {cycle} RR] worker {worker_id} white rows {rows_label} resuming at game {local_games + 1}/{total_games}.")
...
def _cycle_rr_aggregate_worker_progress(
    *,
    ...
    for worker_id, rows_raw in enumerate(chunks, start=1):
        rows = [int(x) for x in rows_raw]
        progress = _load_cycle_rr_worker_progress(
            base_dir=base_dir,
            cycle=cycle,
            worker_id=worker_id,
...
def run_cycle_champions_round_robin(
    ...
    *,
    thread_mode: str = "5",
    thread_groups_override: Optional[Sequence[Sequence[str]]] = None,
    ...
    chunks = _cycle_rr_thread_group_chunks(agent_names, thread_groups_override, thread_mode)
    ...
    log(
        ...
        + ", ".join(
            f"W{idx + 1}=white rows {_cycle_rr_rows_label(rows, agent_names)}"
    ...
    if worker_count == 1:
        ...
        try:
            ...
            for worker_id, rows in enumerate(chunks, start=1):
                rows = [int(x) for x in rows]
                rows_label = _cycle_rr_rows_label(rows, agent_names)
                safe_rows_label = re.sub(r"[^A-Za-z0-9_.-]+", "-", rows_label).strip("-") or str(worker_id)
                p = multiprocessing.Process(
                    ...
                    name=f"CycleRR-{safe_rows_label}",
                )
                processes.append(p)
            ...
            for item in results:
                ...
                rows_label = str(item.get("rows_label") or _cycle_rr_rows_label(item.get("white_indices", []), agent_names))
                err = item.get("error")
                if item.get("stopped"):
                    log(f"[cycle {cycle} RR] worker {wid} white rows {rows_label} stopped cleanly ({item.get('games', 0)} games saved).")
                elif err:
                    log(f"[cycle {cycle} RR] worker {wid} white rows {rows_label} failed: {err}")
                else:
                    log(f"[cycle {cycle} RR] worker {wid} white rows {rows_label} finished ({item.get('games', 0)} games).")
